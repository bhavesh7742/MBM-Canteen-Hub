...
// @desc    Delete/Cancel user order and restore inventory if pending/preparing/ready
...
const deleteMyOrder = async (req, res) => {
    try {
        ...
        // Restore inventory if the order was not finalized (i.e. is pending, preparing, or ready)
        if (['pending', 'preparing', 'ready'].includes(order.status)) {
            for (const item of order.items) {
                const dish = await Dish.findById(item.dishId);
                if (dish) {
                    dish.inventoryQuantity += item.quantity;
                    dish.available = true; // Mark as available again since we added inventory
                    await dish.save();
                }
            }
            // Emit a menuUpdated socket event to sync client inventories in real time
            const io = req.app.get('io');
            if (io) {
                io.emit('menuUpdated');
            }
        ...
        // Emit order status update/deletion to socket
        const io = req.app.get('io');
        if (io) {
...
module.exports = { placeOrder, getMyOrders, getOrderById, deleteMyOrder };