...
<div align="center">
  ...
  [![Amazon CloudFront](https://img.shields.io/badge/Amazon_CloudFront-D05C4C?style=for-the-badge&logo=amazonwebservices&logoColor=white)](https://aws.amazon.com/cloudfront/)
  ...
  <p align="center">
    ...
    Powered by <b>AWS Lambda</b>, <b>Amazon API Gateway</b>, <b>Amazon S3 + CloudFront</b>, and <b>MongoDB Atlas</b>, 
  ...
  <h4>
    ...
    <a href="DEPLOYMENT.md">Production Serverless Deployment Guide</a>
  </h4>
...
- **Backend:** Node.js, Express.js with `serverless-http` wrapper
- **Database:** MongoDB Atlas (Cloud-managed Document database with cached Mongoose connections)

### AWS Serverless Cloud Stack
- **Compute:** AWS Lambda (Node.js 20.x runtime, stateless execution)
- **API Routing:** Amazon API Gateway (HTTP API with CORS and proxy integration)
- **Frontend Hosting:** Amazon S3 (Static asset hosting bucket)
- **Content Delivery:** Amazon CloudFront (Global CDN with HTTPS and SPA error-routing)
...
```mermaid
graph TD
    User[👥 Students & Staff] -->|HTTPS| CF[⚡ Amazon CloudFront CDN]
    CF -->|Static Assets| S3[📦 Amazon S3 Bucket - React SPA]
    
    User -->|API Requests: /api/*| APIGW[🌐 Amazon API Gateway HTTP API]
    APIGW -->|Proxy Integration /{proxy+}| Lambda[⚡ AWS Lambda Function - Express App]
...
- `FRONTEND_URL` - CloudFront domain or local origin allowed by CORS (e.g. `http://localhost:5173`)
...
4. **Amazon S3 & CloudFront:** Build the React frontend (`npm run build`), upload `frontend/dist/` to an S3 bucket, and point a CloudFront distribution with SPA routing (403/404 → `/index.html`) at the bucket.
