...
const ManageMenu = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editDish) {
                ...
                showToast(`Dish "${form.name}" updated successfully!`, 'success');
            } else {
                await API.post('/admin/dishes', payload);
                showToast(`Dish "${form.name}" added successfully!`, 'success');
            }
            setShowModal(false);
            fetchDishes();
        } catch (err) {
    ...
    const handleDelete = async (id, dishName) => {
        const isConfirmed = await showConfirm(
            'Delete Dish',
            `Are you sure you want to delete "${dishName}"? This action is permanent.`
        );
        if (!isConfirmed) return;

        try {
            await API.delete(`/admin/dishes/${id}`);
            showToast(`Dish "${dishName}" deleted.`, 'info');
            fetchDishes();
        } catch (err) {
                            ...
                            <tr key={dish._id}>
                                ...
                                <td>
                                    <div className="actions">
                                        ...
                                        <button className="btn btn-sm btn-danger" onClick={() => handleDelete(dish._id, dish.name)}>Delete</button>
                                    </div>
                                </td>
                            </tr>
                        ))}
